...
<template>
  <div class="w-auto flex flex-col space-y-2 pb-4">
    <div
        class="alert alert-warning flex whitespace-nowrap h-12 w-auto overflow-x-scroll justify-self-center items-center">
      ...
      <div class="form-control w-52">
        <label class="cursor-pointer flex space-x-3 justify-self-center items-center">
          <span class="label-text font-bold">配置锁</span>
          <input
              :checked="!editable" class="toggle toggle-secondary" type="checkbox"
              @click="setEditable(!editable);"/>
          <span class="label-text font-bold">{{ editable ? "编辑中 【关闭编辑模式以保存】" : "锁定中" }}</span>
        </label>
      </div>
    </div>
